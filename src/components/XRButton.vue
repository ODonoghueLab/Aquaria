<template>
  <div v-if="this.dataReceived" class="xr-button">
    ...
    <!-- Other (Download GLTF) -->
    <a v-if="!this.sceneViewer && !this.quickLook" class="xr-platform-button default" :href="this.baseUri + '.glb'" :download="this.proteinId + '.' + this.pdbId + '.glb'">
      <img class="xr-image" src="/images/ar-button.png">
    </a>
  </div>
</template>
...
const MODEL_SERVER = 'https://ie.csiro.au/services/pdbexport'
...
export default {
  ...
  mounted: function () {
    ...
    const chainSelectionProxy = (accession, pdb, chain) => {
      console.log(`XRButton proxy ${accession} ${pdb} ${chain}`)
  ...
  computed: {
    ...
    sceneViewerUri () {
      ...
      const file = `${this.baseUri}.glb` // server generated file URI
