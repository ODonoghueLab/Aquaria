...
export function createFeatureMap (datum) {
  ...
  this.width = document.getElementById('structure-viewer').offsetWidth  - window.AQUARIA.margin.right - window.AQUARIA.margin.left
  this.height = 40 - window.AQUARIA.margin.top - window.AQUARIA.margin.bottom + 35 // height
  this.datum = datum
  var outerdiv = d3.select('#selectedFeature').append('div').attr('id', 'outerFeatureMap')
  this.drawTrack(this.datum, this.createSVGforFeature(outerdiv, this.width, this.height + 10, this.width + 1))
