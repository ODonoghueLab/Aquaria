...
function toDescAndAddToAdedFeat () { // convert to description and add to added feature's description
  ...
  for (const residue in variantResidues) {
    ...
    for (const serverName in variantResidues[residue]) {
    ...
    features.forEach(function (feature) {
      if (featureRegex.test(feature)) {
        ...
        if (feature.split(residue)[1].toLowerCase() == 'ter' || feature.split(residue)[1].toLowerCase() == 'x') {
          ...
          const theUserRes = feature.split(residue)[1]
...
function extractVariantInfoFromUniprot (uniprotData) {
  return new Promise(function (resolve, reject) {
    ...
    const variants_featTypesOfInt = ['Modified residue', 'Mutagenesis site', 'Sequence conflict', 'Sequence variant', 'Cross-link', 'Site', 'Metal ion-binding site']
